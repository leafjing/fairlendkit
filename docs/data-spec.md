...
- `population_definition` (`str`): non-blank description of the population and
  sampling frame included in the audit.
- `score_type`: `probability` or `ranking`; the toolkit does not infer calibration.
- `dataset_version` and `model_version`: non-blank identifiers or explicit `null`
  when unavailable.
- `data_as_of`: extraction/as-of timestamp or explicit `null` when unavailable;
  this remains distinct from the audit execution timestamp.
